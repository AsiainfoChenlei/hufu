...
<div class="sop-dashboard-wrapper" :style="{width: `${wrapperWidth}px`}">
  ...
  <div class="chart-wrapper" id="chart-wrapper" :style="{height: `${chartHeight}px`}" ref='posrel'>
    ...
    <i></i>
    <div v-show="fade">
      <p>{{recordDaylisten.startDate| date-format('ll')}} ~ {{recordDaylisten.endDate|date-format('ll') }}</p>
      <ul>
        <li v-for="(item, index) in recordDay " :key="index">
          <span>{{item.weekDay}}</span>
          <span>{{item.date | formatTime}}</span>
...
import { getstrategies, getCalls, getdayListens } from '../../../api/dashboard'
export default {
  data() {
    return {
      tableData: [],
  ...
  created() {
    ...
    getdayListens().then(res => {
      this.recordDaylisten = res
