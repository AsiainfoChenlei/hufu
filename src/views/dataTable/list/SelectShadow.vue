<template>
  <div class="more-info">
    ...
    <div class="item">
      ...
      <div class="right">
        <el-select v-model="recordContentType" placeholder="请选择" size="mini">
          <el-option
            v-for="(item, index) in recordTypes"
            :key="item.id"
            :label="item.name"
            :value="item.id">
          </el-option>
...
<script>
  ...
  export default {
    ...
    computed: {
      ...
      recordContentType: {
        get() {
          return this.$store.state.qualityGrade.recordContentType
        },
        set(value) {
          this[types.SETRECORDCONTENTTYPE](value)
        }
      },
      recordType: {
        get() {
