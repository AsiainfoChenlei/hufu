...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      // showShare控制分享按钮不显示
      goDetail(row, event) {
        if (row.recordContentType === '虎符' && event.target.tagName.toLowerCase() === 'div') {
          sessionStorage.setItem('record', location.pathname)
          this.$router.push(`/audioPlay?showDetail=${row.recordId}&showShare=false`)
        } else if (row.recordContentType === '质检' && event.target.tagName.toLowerCase() === 'div') {
          sessionStorage.setItem('record', location.pathname)
          this.$router.push(`/quality?showDetail=${row.recordId}&showShare=false`)
        }
      },
      goAudioPlay() {
