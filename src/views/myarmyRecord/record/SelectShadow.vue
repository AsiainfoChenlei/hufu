<template>
    ...
    <div class="item">
			...
			<div class="right">
				<el-select 
					v-model="matchStrategyFlag" 
					placeholder="请选择" 
					size="mini">
					<el-option
						v-for="item in armyIdValue"
						:key="item.id"
						:label="item.name"
						:value="item.id">
					</el-option>
...
export default {
  ...
  computed: {
    ...
    matchStrategyFlag: {
      get() {
        return this.$store.state.record.matchStrategyFlag
      },
      set(value) {
        this[types.SETARMYID](value)
      }
    },
    ...mapGetters('record', [
      'enterpriseIdValue',
