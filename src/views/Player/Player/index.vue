...
import Slider from './Slider' //  单独的进度条
...
export default {
  ...
  methods: {
    ...
    currentChange(val) {  //  点击进度条、调整当前时间
    ...
    showErrorConfirm(item) {
      ...
      if (this.playerMode === 'score') {
        ...
        this.$modal.confirm({
          ...
          // window.console.log('点击后出现', res, item)
          if (res.type === 'modify') {
            // 点击了修改, 进行人工校验
            ...
            // 存在 item 说明是系统判定的违规
            // 否则为用户手动点击
            // window.console.log('违规判断', item)
            if (item) {
              this.$emit('confirm', item) //  这个点有记录、添加到质检信息表
