    ...
    <div class="header-wrapper">
      ...
      <left-bar v-if="playerMode !== 'detail'"></left-bar>
        ...
        <div class="right-content">
          ...
          <Evaluate></Evaluate>
          <quality-info v-show="musicData.matchStrategyFlag == 1"  :studentAndOrder="studentAndOrder"></quality-info>
          <quality-grade v-show="musicData.matchStrategyFlag == 1"></quality-grade>
          <save-footer :DetailsType="'hufu'" v-if="playerMode !== 'detail'"></save-footer>
          <Illegal
            v-if="showRuleBreak"
            :errorId="errorId"
...
import Evaluate from './QualityGrade/Evaluate'   //  是否扣流水
import IllegalList from './QualityGrade/IllegalList' //  违规记录
export default {
  components: {
    HeadBar,  //  head导航栏
    LeftBar,  //  侧边录音列表
    DetailInfo, //  学生报名信息、订单信息
    QualityInfo,
    QualityGrade,
    SaveFooter,
    Illegal,
  ...
  methods: {
    ...
    confirm(item) {
      confirmBadByjiqi(this.currentId, item.id).then(() => {  //  机器判定违规点、点确定保存在后台
