 ...
 * @param {当前状态} state
 */
const refreshOneRecord = state => {   //  切换录音不会触发保存
 ...
 * 加载中状态除外
 */
const lockTheSaveLock = state => {  //  数据变化、可以保存。
  if (!state.isLoading) state.saveLock = true
}
export default {
  [types.HANDLE_NEXT](state) {  //  下一个、切换录音索引的位置
  ...
  [types.HANDLE_PREV](state) {  //  上一个、切换录音索引的位置
    const index = findCurrentIndex(state)
  ...
  [types.GET_ARRS](state, value) {  //  修改arrs的值、未使用
    state.arrs = value
  },
  // 路由的违规时间点加红点
  ...
  [types.SET_DETAILSTYPE](state, value) { //  质检或是虎符页面赋值
    state.DetailsType = value
  },
  // ==========  下拉框逻辑  ====================================================
  ...
  [types.SET_FINISHNUM](state, value) {
    state.finishNum = value.toString()
    lockTheSaveLock(state)
  },
  ...
  // ========================================================
  [types.SET_ERRORS](state, value) {  //  updata录音的违规消息列表
    // console.log('质检消息队列', value)
    state.errorsList = value
  },
  [types.SET_DELERRORS](state, value) {  //  点击取消的时候删除当前列表展示录音
    var lists = state.errorsList.filter(res => {
      if (res.id !== value) {
        return res
      }
    })
    state.errorsList = lists
  },
  [types.HANDLE_ADDERRORS](state, value) {
    // console.log('时间点', state.errorsList, value)
    for (var k of state.errorsList) { // 如果时间在一样的位置、覆盖原内容
      if (k.badTime === value.badTime) {
  ...
  [types.SET_MUSIC](state, value) { //  action传进来的录音数据
    value.errors = value.errors.map(item => {
      return Object.assign({}, item, {
        showed: false
      })
    })
    // window.console.log('audio的数据', value)
