...
export default {
  ...
  async [types.GET_BASEINFO]({ commit }) {  //  虎符质检打分表的数据
    commit(types.SET_LOADING, true) //  加载样式
    window.console.log('请求的数据', await selectRecordDetails())
    const [
			{
        studentAndOrder,
        checkInfo,
   ...
   * 2. 全部提交前
   */
  [types.HANDLE_SAVESCOREINFO]({ commit, state, dispatch }, isSubmit = false) { //  下一步的时候进行验证
    const data = {
      aim: state.aim,
      ...
      recordContentType: 1  //  虎符进来
    }
    const qualityData = {
      timeLength: state.timeLength, //  时长
      recType: state.musicData.recType, //  录音状态
      recordId: state.currentId,  //  录音id
      isWater: state.isWater, // 是否扣除流水
      forfeit: state.forfeit, // 罚款金额
      recordContentType: 0  //  质检进来
    }
    if (state.DetailsType === 'quality' || state.musicData.matchStrategyFlag === '0') { //  只有质检和没有军团策略才能进来
      // console.log('qualityData', qualityData)
      for (var k in qualityData) {
        if (!qualityData[k] === '') {
          // Message({
          //   type: 'error',
          //   message: '请填写完整',
      ...
      for (const i in data) {
        // 不是必填项
        if (i === 'goodType' || i === 'majorTypeOneName' || i === 'majorTypeTwoName' || i === 'satisfactionNum' || i === 'sex' ||
        i === 'professionalDegreeNumOne' || i === 'professionalDegreeNumTwo' || i === 'professionalDegreeNumThree' ||
          i === 'professionalDegreeNumFour' || i === 'ageStage' || i === 'degree' || i === 'consultDegreeStage' || i === 'questions') continue
        if (i === 'isGood') {
          if (data[i] === '1') {
            if (!data['goodType']) {
              Message({
    ...
    return new Promise((resolve, reject) => {
      addRecordScoreToSave((state.DetailsType === 'quality' || state.musicData.matchStrategyFlag === '0') ? qualityData : data).then(res => {
  ...
  async [types.HANDLE_NEXT]({ commit, dispatch, state }, value) {  //  点击下一步
    if (state.DetailsType === 'quality' || state.musicData.matchStrategyFlag === '0') { //  只有质检和没有军团策略才能进来保存
      commit(types.HANDLE_SAVELOCK, true)
    }
    if (!state.saveLock || await dispatch(types.HANDLE_SAVESCOREINFO)) {
      commit(types.HANDLE_NEXT)
      commit(types.SET_DETAILSTYPE, value)
      await dispatch(types.GET_MUSIC)
      await dispatch(types.GET_BASEINFO)
      // dispatch(types.FINISHED_MUSIC)
    }
  },
  async [types.HANDLE_PREV]({ commit, dispatch, state }, value, long) {  //  点击上一步
    // console.log('上一个', value)
    if (state.DetailsType === 'quality' || state.musicData.matchStrategyFlag === '0') { //  只有质检和没有军团策略才能进来保存
      commit(types.HANDLE_SAVELOCK, true)
    }
    if (!state.saveLock || await dispatch(types.HANDLE_SAVESCOREINFO)) {
      ...
      commit(types.SET_DETAILSTYPE, value)
      await dispatch(types.GET_MUSIC) //  请求录音数据
      await dispatch(types.GET_BASEINFO)  //  虎符质检打分表的数据
      // dispatch(types.FINISHED_MUSIC)
    }
  },
  async [types.HANDLE_CHOOSEITEM]({ commit, dispatch, state }, id, long) {  //  当前列表录音点击
    if (state.DetailsType === 'quality' || state.musicData.matchStrategyFlag === '0') { //  只有质检和没有军团策略才能进来保存
      commit(types.HANDLE_SAVELOCK, true)
    }
    if (!state.saveLock || await dispatch(types.HANDLE_SAVESCOREINFO)) {
  ...
  async [types.GET_MUSIC]({ state, commit }) {  //  获取录音的信息、时长,并赋值
    // console.log('切换id', state.currentId)
